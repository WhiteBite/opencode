...
const saved = await ctx.worktree.list({ projectID })
const inventory = await ctx.worktree.discover({ projectID })
...
Create and discover wait for the canonical checkout's plugins to activate. Calls made during that checkout's plugin
...
Discover loads canonical configuration and asks all available strategies to find worktrees across known checkout roots.
It combines results, preserves recorded ownership, prunes missing directories, and returns the complete latest saved
inventory. A failed discovery source is logged while other sources continue. Existing directories remain saved even if
no available strategy reports them.
