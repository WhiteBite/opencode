...
Worktree operations require a `projectID`. Create and discover load configuration and plugins from the project's saved
...
const inventory = await opencode.worktree.discover({ projectID })
